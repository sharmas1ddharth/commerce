...
from .models import User, Listing, Watchlist, Alllisting,Bid, Comment, Closedbid
...
def submit(request):
    if request.method == "POST":
        ...
        if request.POST.get("link"):
            ...
            listtable.link = "https://bitsofco.de/content/images/2018/12/broken-1.png"
        listtable.time = dt
        listtable.save()
        all = Alllisting()
        items = Listing.objects.all()
...
def category(request, category):
    category_item = Listing.objects.filter(category=category)
    try:
        w = Watchlist.objects.filter(user=request.user.username)
        wcount = len(w)
    except:
...
def cmtsubmit(request, listingid):
    if request.method == "POST":
        now = datetime.now()
        dt = now.strftime("%d %B %Y %X")
        c = Comment()
